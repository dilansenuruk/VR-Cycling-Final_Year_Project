...
static void notifyCallback(BLERemoteCharacteristic* pBLERemoteCharacteristic,
  ...
  if(pBLERemoteCharacteristic->getUUID().toString() == charUUID_1.toString()) {
    ...
    //for(int i = 1; i<length; i++) {
      //dataVal = dataVal | (pData[i] << i*8);
    //}
    // print to Serial
    String rxValue = String(value);
    
    Serial.println("Characteristic 1 (Notify) from server: "+ rxValue); 

    Serial.println("Characteristic 1 (writeValue): " + rxValue);
    
    // Set the characteristic's value to be the array of bytes that is actually a string.
    pRemoteChar_2->writeValue(rxValue.c_str(),rxValue.length());
    value++;
  }
}

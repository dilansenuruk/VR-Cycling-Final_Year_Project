...
async def udp_send(client_socket, i, timestamps):
    send_bytes = f"Pasindu:{i}:msg:o".encode('ascii')
...
async def main():
    ...
    if delays:
        ...
        delays_less_than_500ms = [delay * 1000 for delay in delays if delay < 0.3]

        # Plot histogram
        plt.hist(delays_less_than_500ms, bins=50, alpha=0.7, color='blue', edgecolor='black')
        ...
        # Calculate statistics for delays less than 500 milliseconds
        mean_delay = statistics.mean(delays_less_than_500ms)
        median_delay = statistics.median(delays_less_than_500ms)
        std_delay = statistics.stdev(delays_less_than_500ms)
        var_delay = statistics.variance(delays_less_than_500ms)
